...
export class FireserviceService {
  ...
  updateCartItem(data, cartId){
    return this.firestore.collection('carts').doc(cartId).collection('items').doc(data.itemId).update(data);
  }

  removeItemsfromCart(data, cartId){
    console.log('Data', data, cartId);

   //return this.firestore.collection('carts').doc(cartId).collection('items').doc(data.itemId).delete();
